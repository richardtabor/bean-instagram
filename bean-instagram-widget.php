...
{
	add_options_page(
		__('Instagram Settings', 'bean'), __('Bean Instagram', 'bean'), 'manage_options', 'bean-instagram', 'bean_instagram_admin_page'
		);
...
function bean_instagram_enqueue_scripts($hook)  {
	...
	if ('settings_page_bean-instagram' !== $hook) return;
...
function bean_instagram_admin_page($id) 
{
	...
	$redirectURI = $prefix . $_SERVER['HTTP_HOST'] . $uri_parts[0] . '?page=bean-instagram';
	...
	echo '<div class="wrap">'; 
	echo '<p>' . __('Display your Instagram photostream with the Bean Instagram plugin.  After entering your codes, simply add the Bean Instagram widget to a widget area within your Widgets Dashboard. If you need additional help, we wrote a <a href="http://themebeans.com/tutorials/retrieving-your-instagram-id-secret/" target="_blank">quick guide</a> to help you along.', 'bean' ) . '</p></br>';
	?>
	<?php
	...
	echo '<li>' . __( 'Visit the <strong><a href="http://instagram.com/developer" target="_blank">Instagram Developer</a></strong> portal and complete the following steps:', 'bean' ) . '</li>';

	/* translators: Click is used as a verb. */

	echo '<ol>';

	printf( '<li>' . __( 'Click the %1$s button on the top bar.', 'bean' ) . '</li>', "<strong>Manage Clients</strong>");

	/* translators: Click is used as a verb. */

	printf( '<li>' . __( 'Click the %1$s button.', 'bean' ) . '</li>', "<strong>Register a New Client</strong>");

	printf( '<li>' . __( 'Enter your application name, description and website.', 'bean' ) . '</li>');

	printf( '<li>' . __( 'Set the %1$s field to: %2$s', 'bean' ) . '</li>' , "<strong>OAuth redirect_uri</strong>", "<strong>$redirectURI</strong>");

	echo '<li>' . __( 'After all fields are complete, click the green <strong>Register</strong> button.', 'bean' ) . '</li>';

	echo '<li>' . __( 'Keep the Instagram Clients page open and head back to this page.', 'bean' ) . '</li>';

	echo '</ol></br>';

	printf( '<li>' . __( 'Copy & paste the %1$s and %2$s codes of your Instagram Client in the OAuth Codes form below.', 'bean' ) . '</li>', "<strong>Client ID</strong>", "<strong>Client Secret</strong>" );
	...
	/* translators: Click the "Retrieve Access Token" button below. */
	...
	printf( '<li>' . __( 'Click the blue %1$s button below the OAuth Codes one last time. Your access token should be "Available".', 'bean' ) . '</li>', "<strong>" . __( 'Save Changes' ) . "</strong>" );
	...
	if (empty($stored_accesstoken) && !isset($accesstoken_value)) {
		...
		echo '<strong id="bean_inst_access_token_status" style="color: #FFF; background-color: #2BC28B; border-radius: 2px; padding: 3px 6px;">Available';
